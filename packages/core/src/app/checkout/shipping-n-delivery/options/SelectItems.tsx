        ...
        {mainCartItems.filter(i => c.lineItemIds.includes(i.id as string))
          .map(i => <div key={i.id} style={{ display: 'flex', alignItems: 'center', gap: '20px'}}>
            ...
            <div style={{ display: 'flex', backgroundColor: '#fff', padding: '10px', borderRadius: '20px', width: "100%" }}>
              ...
              <div style={{ width: '15%', fontWeight: 'bold' }} className="product-price">${i.salePrice}</div>
              
              <div style={{ width: '15%', marginRight: '20px' }}>
                <div onClick={() => setIsEdit(true)} style={{ cursor: 'pointer', border: '1px solid #555', borderRadius: '8px', padding: '5px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }}>
